...
#### Post 실험
async def PostAhuTotalPowerDaily(startDate: str, endDate: str):
    try:
        connection = GetConnection()
        ...
        with connection.cursor() as cursor:
            ...
            # ress = str(rv)
            json_data = json.dumps(rv, indent=4)
            # print(f"{datetime.datetime.now()} : succeed to do 'PostAhuTotalPowerDaily('{startDate}',{endDate})'")
            _logger.Info(
                f"succeed to do 'PostAhuTotalPowerDaily('{startDate}',{endDate})'")
            return json_data

    except Exception as ex:
        # print(f"{datetime.datetime.now()} : error to do 'PostAhuTotalPowerDaily('{startDate}',{endDate})'")
        _logger.Info(f"error to do 'PostAhuTotalPowerDaily('{startDate}',{endDate})'")



async def PostAhuTotalPowerHourly(runDate: str):
    try:
        connection = GetConnection()
        ...
        with connection.cursor() as cursor:
            ...
            # ress = str(rv)
            json_data = json.dumps(rv, indent=4)
            # print(f"{datetime.datetime.now()} : succeed to do 'PostAhuTotalPowerHourly('{runDate}')'")
            _logger.Info(f"succeed to do 'PostAhuTotalPowerHourly('{runDate}')'")
            return json_data

    except Exception as ex:
        # print(f"{datetime.datetime.now()} : error to do 'PostAhuTotalPowerHourly('{runDate}'")
        _logger.Info(f"error to do 'PostAhuTotalPowerHourly('{runDate}'")
